...
static void rosbasesrc_set_property(
  GObject * object, guint property_id, const GValue * value, GParamSpec * pspec);
static void rosbasesrc_get_property(
  GObject * object, guint property_id, GValue * value, GParamSpec * pspec);

static GstStateChangeReturn rosbasesrc_change_state(
...
G_DEFINE_TYPE_WITH_CODE(
  RosBaseSrc, rosbasesrc, GST_TYPE_BASE_SRC,
  GST_DEBUG_CATEGORY_INIT(
    rosbasesrc_debug_category, "rosbasesrc", 0, "debug category for rosbasesrc element"))
...
static void rosbasesrc_class_init(RosBaseSrcClass * klass)
{
  ...
  gst_element_class_set_static_metadata(
    element_class, "rosbasesrc", "Source",
    "a gstreamer source class to place ROS nodes in gstreamer", "BrettRD <brettrd@brettrd.com>");

  g_object_class_install_property(
    object_class, PROP_ROS_NAME,
    g_param_spec_string(
      "ros-name", "node-name", "Name of the ROS node", "ros_base_src_node",
  ...
  switch (transition) {
    ...
    case GST_STATE_CHANGE_PAUSED_TO_PLAYING: {
      ...
      src->ros_clock_offset =
        gst_bridge::sample_clock_offset(GST_ELEMENT_CLOCK(src), src->stream_start);
      break;
    }
