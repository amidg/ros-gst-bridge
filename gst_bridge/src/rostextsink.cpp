...
static void rostextsink_set_property(
  GObject * object, guint property_id, const GValue * value, GParamSpec * pspec);
static void rostextsink_get_property(
...
static GstFlowReturn rostextsink_render(
...
static GstStaticPadTemplate rostextsink_sink_template =
  GST_STATIC_PAD_TEMPLATE("sink", GST_PAD_SINK, GST_PAD_ALWAYS, GST_STATIC_CAPS(ROS_TEXT_MSG_CAPS));

/* class initialization */

G_DEFINE_TYPE_WITH_CODE(
  Rostextsink, rostextsink, GST_TYPE_ROS_BASE_SINK,
  GST_DEBUG_CATEGORY_INIT(
    rostextsink_debug_category, "rostextsink", 0, "debug category for rostextsink element"))
...
static void rostextsink_class_init(RostextsinkClass * klass)
{
  ...
  gst_element_class_add_static_pad_template(element_class, &rostextsink_sink_template);

  gst_element_class_set_static_metadata(
    element_class, "rostextsink", "Sink", "a gstreamer sink that publishes text data into ROS",
    "BrettRD <brettrd@brettrd.com>");

  g_object_class_install_property(
    object_class, PROP_ROS_TOPIC,
    g_param_spec_string(
      "ros-topic", "pub-topic", "ROS topic to be published on", "gst_text_pub",
  ...
  ros_base_sink_class->open =
    GST_DEBUG_FUNCPTR(rostextsink_open);  //let the base sink know how we register publishers
  ros_base_sink_class->close =
    GST_DEBUG_FUNCPTR(rostextsink_close);  //let the base sink know how we destroy publishers
  ros_base_sink_class->render =
    GST_DEBUG_FUNCPTR(rostextsink_render);  // gives us a buffer to package
}
