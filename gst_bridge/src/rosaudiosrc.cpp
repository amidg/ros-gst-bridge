...
static void rosaudiosrc_set_property(
  GObject * object, guint property_id, const GValue * value, GParamSpec * pspec);
static void rosaudiosrc_get_property(
...
static GstFlowReturn rosaudiosrc_create(
...
static GstCaps * rosaudiosrc_getcaps(
  GstBaseSrc * gst_base_src, GstCaps * filter);  //set our caps preferences
...
static void rosaudiosrc_set_msg_props_from_msg(
  Rosaudiosrc * src, audio_msgs::msg::Audio::ConstSharedPtr msg);
...
static GstStaticPadTemplate rosaudiosrc_src_template =
  GST_STATIC_PAD_TEMPLATE("src", GST_PAD_SRC, GST_PAD_ALWAYS, GST_STATIC_CAPS(ROS_AUDIO_MSG_CAPS));

/* class initialization */

G_DEFINE_TYPE_WITH_CODE(
  Rosaudiosrc, rosaudiosrc, GST_TYPE_ROS_BASE_SRC,
  GST_DEBUG_CATEGORY_INIT(
    rosaudiosrc_debug_category, "rosaudiosrc", 0, "debug category for rosaudiosrc element"))
...
static void rosaudiosrc_class_init(RosaudiosrcClass * klass)
{
  ...
  gst_element_class_add_static_pad_template(element_class, &rosaudiosrc_src_template);

  gst_element_class_set_static_metadata(
    element_class, "rosaudiosrc", "Source/Audio",
  ...
  g_object_class_install_property(
    ...
    g_param_spec_string(
      "ros-topic", "sub-topic", "ROS topic to subscribe to", "gst_audio_sub",
  ...
  ros_base_src_class->open =
    GST_DEBUG_FUNCPTR(rosaudiosrc_open);  //let the base sink know how we register publishers
  ros_base_src_class->close =
    GST_DEBUG_FUNCPTR(rosaudiosrc_close);  //let the base sink know how we destroy publishers
  ...
  basesrc_class->fixate =
    GST_DEBUG_FUNCPTR(rosaudiosrc_fixate);  //set caps fields to our preferred values (if possible)
  ...
  Rosaudiosrc * src, audio_msgs::msg::Audio::ConstSharedPtr msg)
{
  ...
  if (0 == g_strcmp0(src->encoding, ""))
    fmt = gst_bridge::getGstAudioFormat(msg->encoding);
  else
    fmt = gst_bridge::getGstAudioFormat(src->encoding);

  gst_audio_info_set_format(&(src->audio_info), fmt, msg->sample_rate, msg->channels, NULL);

  if ((uint32_t)GST_AUDIO_INFO_BPF(&(src->audio_info)) != msg->step)
    RCLCPP_ERROR(
      ros_base_src->node_if->logging->get_logger(), "audio format misunderstood, step %d != %d",
  ...
  if (
    ...
    RCLCPP_ERROR(
      ros_base_src->node_if->logging->get_logger(), "audio format misunderstood, layout %d != %d",
      GST_AUDIO_INFO_LAYOUT(&(src->audio_info)),
      ((msg->layout == audio_msgs::msg::Audio::LAYOUT_INTERLEAVED)
         ? GST_AUDIO_LAYOUT_INTERLEAVED
         : GST_AUDIO_LAYOUT_NON_INTERLEAVED));
...
static gboolean rosaudiosrc_open(RosBaseSrc * ros_base_src)
{
  ...
  src->sub = rclcpp::create_subscription<audio_msgs::msg::Audio>(
...
static GstCaps * rosaudiosrc_fixate(GstBaseSrc * gst_base_src, GstCaps * caps)
{
  ...
  if (gst_structure_has_field(s, "signed")) gst_structure_fixate_field_boolean(s, "signed", TRUE);
  ...
  if (ros_base_src->node_if)
    RCLCPP_INFO(
      ros_base_src->node_if->logging->get_logger(), "preparing audio with caps '%s'",
...
*/
  ...
  if (ros_base_src->node_if)
    RCLCPP_INFO(
      ros_base_src->node_if->logging->get_logger(), "getcaps with filter '%s'",
  ...
  if (0 == g_strcmp0(src->init_caps, "")) {
    ...
    rosaudiosrc_set_msg_props_from_msg(
      src, msg);  //XXX generalise this to return audio_info instead of relying on side-effects
  ...
  GstBaseSrc * gst_base_src, guint64 offset, guint size, GstBuffer ** buf)
{
  ...
  if (*buf == NULL) {
    ...
    ret =
      GST_BASE_SRC_CLASS(rosaudiosrc_parent_class)->alloc(gst_base_src, offset, length, &res_buf);
