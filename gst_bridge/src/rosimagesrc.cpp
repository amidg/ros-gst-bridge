...
static void rosimagesrc_set_property(
  GObject * object, guint property_id, const GValue * value, GParamSpec * pspec);
static void rosimagesrc_get_property(
...
static GstFlowReturn rosimagesrc_create(
...
static GstCaps * rosimagesrc_getcaps(
  GstBaseSrc * base_src, GstCaps * filter);  //set our caps preferences
...
static GstStaticPadTemplate rosimagesrc_src_template =
  GST_STATIC_PAD_TEMPLATE("src", GST_PAD_SRC, GST_PAD_ALWAYS, GST_STATIC_CAPS(ROS_IMAGE_MSG_CAPS));

/* class initialization */

G_DEFINE_TYPE_WITH_CODE(
  Rosimagesrc, rosimagesrc, GST_TYPE_ROS_BASE_SRC,
  GST_DEBUG_CATEGORY_INIT(
    rosimagesrc_debug_category, "rosimagesrc", 0, "debug category for rosimagesrc element"))
...
static void rosimagesrc_class_init(RosimagesrcClass * klass)
{
  ...
  gst_element_class_add_static_pad_template(element_class, &rosimagesrc_src_template);

  gst_element_class_set_static_metadata(
    element_class, "rosimagesrc", "Source/Video",
  ...
  g_object_class_install_property(
    ...
    g_param_spec_string(
      "ros-topic", "sub-topic", "ROS topic to subscribe to", "gst_image_sub",
  ...
  basesrc_class->fixate =
    GST_DEBUG_FUNCPTR(rosimagesrc_fixate);  //set caps fields to our preferred values (if possible)
...
static void rosimagesrc_set_msg_props_from_caps_string(Rosimagesrc * src, gchar * caps_string)
{
  ...
  if (!format_str) GST_DEBUG_OBJECT(src, "caps_init missing format");
...
static GstCaps * rosimagesrc_fixate(GstBaseSrc * base_src, GstCaps * caps)
{
  ...
  if (ros_base_src->node_if)
    RCLCPP_INFO(
      ros_base_src->node_if->logging->get_logger(), "preparing video with caps '%s'",
  ...
  if (src->msg_init) {
    ...
    if (!(src->step == msg->step / msg->width))
      RCLCPP_ERROR(
        ...
        "image format changed during playback, step %ld != %d", src->step, msg->step / msg->width);
    if (!(src->height == (int)msg->height))
      RCLCPP_ERROR(
        ros_base_src->node_if->logging->get_logger(),
        "image format changed during playback, height %d != %d", src->height, msg->height);
    if (!(src->width == (int)msg->width))
      RCLCPP_ERROR(
        ros_base_src->node_if->logging->get_logger(),
        "image format changed during playback, width %d != %d", src->width, msg->width);
    if (!(src->endianness == (msg->is_bigendian ? G_BIG_ENDIAN : G_LITTLE_ENDIAN)))
      RCLCPP_ERROR(
        ros_base_src->node_if->logging->get_logger(),
        "image format changed during playback, endianness %d != %d", src->endianness,
